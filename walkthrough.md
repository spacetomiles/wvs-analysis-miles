...
![Feature Importance Plot](/Users/a11/Downloads/HKU/7005/7005%20group%20project/feature_importance_plot.svg)
...
![Ordinal Regression Feature Importance](feature_importance_ordinal.svg)

A detailed table of coefficients has been saved to `regression_coefficients.csv`.
...
| **Q46** | Happiness | 0.54 | 1.72 | **Recoded**: Higher values now mean "Happier". Strong positive predictor. |
...
| **Q110** | Confidence: The Press | -0.04 | 0.96 | Lower confidence is slightly associated with higher satisfaction. |
...
| **Q47** | Health | 0.13 | 1.14 | **Recoded**: Higher values now mean "Better Health". Good health predicts satisfaction. |
| **Q213** | I see myself as: Reserved | -0.01 | 0.99 | Being more reserved is slightly associated with lower satisfaction. |
...
![Binary ROC Curve](/Users/a11/Downloads/HKU/7005/7005%20group%20project/roc_curve_ordinal_binary.svg)
...
## Files Created
-   `master_pipeline.py`: **[NEW]** The single complete script that runs the entire analysis from start to finish.
-   `clean_wvs_data.py`: Data cleaning script (legacy).
-   `verify_wvs_data.py`: Data verification script (legacy).
-   `run_modeling_pipeline.py`: Modeling pipeline script (legacy).
-   `run_targeted_regression.py`: Targeted regression script (legacy).
